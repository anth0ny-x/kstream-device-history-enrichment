...
public class WordCountLambdaExample {
  ...
  static final String inputTopic = "test-topic-8";
  ...
  public static void main(final String[] args) {
    ...
    createWordCountStreamWindowed(builder);
  ...
  static Properties getStreamsConfiguration(final String bootstrapServers) {
    ...
    streamsConfiguration.put(StreamsConfig.CLIENT_ID_CONFIG, "wordcount-lambda-example2-client");
...
//  static void createWordCountStreamWindowed(final StreamsBuilder builder) {
//    // Construct a `KStream` from the input topic "streams-plaintext-input", where message values
//    // represent lines of text (for the sake of this example, we ignore whatever may be stored
//    // in the message keys).  The default key and value serdes will be used.
//    final KStream<String, String> textLines = builder.stream(inputTopic);
...
//    final Pattern pattern = Pattern.compile("\\W+", Pattern.UNICODE_CHARACTER_CLASS);
//    
//    //final KTable<Windowed<java.lang.String>, Long> wordCounts = textLines
//    final KTable<String, Long> wordCounts = textLines
//      // Split each text line, by whitespace, into words.  The text lines are the record
//      // values, i.e. we can ignore whatever data is in the record keys and thus invoke
//      // `flatMapValues()` instead of the more generic `flatMap()`.
...
//    wordCounts.toStream().foreach((key, val) -> System.out.println(key + " " + val));
//  }
  ...
  static void createWordCountStreamWindowed(final StreamsBuilder builder) {
	...
	KStream<Windowed<String>, Long> windowedCount = groupedByWord.windowedBy(window)
    	.count(
    		Materialized.<String, Long, WindowStore<Bytes, byte[]>>as("windowed-word-count2")
    		//.withValueSerde(Serdes.Long())
    	 )
    	.toStream();
    	
	windowedCount.foreach((key, val) -> System.out.println(key + " " + val));

//	TimeWindowedSerde<String> windowSerde = new TimeWindowedSerde<>(Serdes.String(), window.size());
//	windowedCount.to(outputTopic, Produced.with(windowSerde, Serdes.Long()));
	
  }
  
//  static void createCollectListStreamWindowed(final StreamsBuilder builder) {
//	  final KStream<String, String> textLines = builder.stream(inputTopic);
//	  final Pattern pattern = Pattern.compile("\\W+", Pattern.UNICODE_CHARACTER_CLASS);
//	  final KGroupedStream<String, String> groupedByWord = textLines
//			  .flatMapValues(value -> Arrays.asList(pattern.split(value.toLowerCase())))
//			  .groupBy((key, word) -> word, Grouped.with(Serdes.String(), ));
//	  
//	  TimeWindows window = TimeWindows.of(Duration.ofMinutes(1));
//	  KStream<Windowed<String>, String> windowedCount = groupedByWord.windowedBy(window)
//			  .aggregate(null, null)
//	    	.toStream();
//  }
